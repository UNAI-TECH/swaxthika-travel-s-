...
import { ChevronRight, ChevronLeft, Clock, MapPin, Sparkles, ArrowRight, BookOpen } from 'lucide-react';
...
export const PopularTemples: React.FC<PopularTemplesProps> = ({
  ...
  const handleNext = () => {
    setCurrentIndex((prev) => (prev + 1) % Math.max(1, temples.length - 3));
  };

  const handlePrev = () => {
    setCurrentIndex((prev) => (prev - 1 + Math.max(1, temples.length - 3)) % Math.max(1, temples.length - 3));
  };
    ...
    <div className="space-y-6">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-xs text-[#534341] mt-0.5">
            Explore sacred shrines, live darshan wait times, and spiritual heritage.
          </p>
        </div>
        <button
      ...
      <div className="relative">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {temples.slice(currentIndex, currentIndex + 4).map((temple) => {
            const crowd = getCrowdBadge(temple.crowdLevel);
            return (
              <div
                key={temple.id}
                onClick={() => onSelectTemple(temple)}
                className="bg-white rounded-lg overflow-hidden card-shadow group cursor-pointer border border-gray-100/80 hover:border-[#c5a059]/40 transition-all duration-300 transform hover:-translate-y-1"
              >
                <div className="h-40 overflow-hidden relative">
                  <img
                    ...
                    src={temple.image}
                  />
                  <div className="absolute top-2 right-2 bg-black/50 backdrop-blur-xs text-white px-2 py-0.5 rounded text-[10px] flex items-center gap-1">
                ...
                <div className="p-3.5 flex flex-col justify-between h-[110px]">
                  <div>
                    <h4 className="font-bold text-xs text-gray-900 line-clamp-1 group-hover:text-[#5d100a] transition-colors">
                      {temple.name}
                    </h4>
        ...
        {temples.length > 4 && (
          <button
            onClick={handleNext}
            className="absolute -right-4 top-1/2 -translate-y-1/2 w-8 h-8 bg-white border border-gray-200 rounded-full flex items-center justify-center shadow-lg hover:bg-[#fff8f5] hover:border-[#c5a059] transition-all cursor-pointer z-10"
            title="Next Temples"
          >
